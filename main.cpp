    ...
    while(!inputFile.eof())
    {
      ...
      for(int i = 0; i < counter; i++)
      {
        ...
        cout.imbue(std::locale("en_US.UTF-8"));  //allows me to format these numbers as USD
                cout << setw(15) << fixed << showpoint << setprecision(2) << pmtReq[i].advance.appliedAgainst;  //show in terminal what you're applying
          
          outputFile << setw(15) << fixed << showpoint << setprecision(2) << pmtReq[i].advance.appliedAgainst;  // output it to file too
